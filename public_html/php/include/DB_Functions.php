...
class DB_Functions {
     ...
     */
    public function storeUser($name, $userid, $password, $grade, $email) {
        $uuid = uniqid('', true);
        ...
        if ($result) {
            $stmt = $this->conn->prepare("SELECT * FROM rom_staff WHERE userid = ?");
    ...
    public function getUserByEmailAndPassword($userid, $password) {
        ...
        $userid=trim($userid);
        //$stmt = $this->conn->prepare("SELECT * FROM rom_staff WHERE userid = ?");
        $stmt = $this->conn->prepare("SELECT * FROM rom_staff WHERE userid = '".$userid."'");
    ...
    public function isUserExisted($userid) {
        $stmt = $this->conn->prepare("SELECT userid from rom_staff WHERE userid = ?");
